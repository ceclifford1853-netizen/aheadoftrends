...
export function ScarcityCounter({
  ...
  return (
    <div className="inline-block">
      <div
        className={`bg-gradient-to-r ${isUrgent ? "from-pink-500/20 to-red-500/20 border-pink-500/50" : "from-cyan-500/20 to-blue-500/20 border-cyan-500/50"} border rounded-lg p-4 backdrop-blur-sm transition-all duration-300 ${isAnimating ? "scale-105" : "scale-100"}`}
      >
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <Zap
              className={`w-5 h-5 ${isUrgent ? "text-pink-500 animate-pulse" : "text-cyan-400"}`}
            />
            <div>
              <p
                className={`text-sm font-semibold ${isUrgent ? "text-pink-500" : "text-cyan-400"}`}
              >
