...
const AGENT_NODES: AgentNode[] = [
  {
    ...
    label: "Scout",
    icon: "🔍",
    description: "Scanning real-time AEO signals & trend data",
  },
  {
    id: "architect",
    label: "Architect",
    icon: "🏗️",
    description: "Structuring EEAT-optimized content framework",
  },
  {
    id: "ghostwriter",
    label: "Ghostwriter",
    icon: "✍️",
    description: "Generating high-authority technical content",
  },
  {
    id: "auditor",
    label: "Auditor",
    icon: "✅",
    description: "Validating AI-readiness & accuracy",
  },
];
export function AgenticStatusTracker({
  isActive = false,
}: {
  isActive?: boolean;
}) {
        ...
        {AGENT_NODES.map(node => {
          ...
          return (
            ...
            >
                ...
                className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl border-2
                  ${
                    isComplete
                      ...
                      : isActiveStage
                        ? "bg-cyan-500/20 border-cyan-300 text-cyan-100 animate-pulse"
              ...
              <span
                className={`mt-3 text-sm font-medium font-dm-mono
