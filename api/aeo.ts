...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const externalLinkCount = $('a[href^="http"]').not(
      `[href*="${hostname}"]`
    ).length;
    ...
    schemas.forEach((s: any) => {
      if (s["@type"]) schemaTypesSet.add(String(s["@type"]));
    });
    ...
    if (wordCount < 200)
      findings.push(`Low word count (${wordCount} words). Target 800+ words.`);
    else if (wordCount < 500)
      findings.push(
        `Moderate word count (${wordCount} words). Expand to 800+.`
      );
    if (h1Count === 0)
      findings.push("Missing H1 tag. Every page needs exactly one H1.");
    else if (h1Count > 1)
      findings.push(`Multiple H1 tags (${h1Count}). Use only one H1 per page.`);
    if (h1Count > 0 && h2Count === 0)
      findings.push("No H2 subheadings. Use H2s for scannable sections.");
    if (altRatio < 0.8 && imageCount > 0)
      findings.push(
        `Only ${Math.round(altRatio * 100)}% of images have alt text.`
      );
    if (internalLinkCount < 3)
      findings.push(
        `Only ${internalLinkCount} internal links. Add 3+ for site context.`
      );
    if (!title) findings.push("Missing page title tag.");
    else if (title.length < 30 || title.length > 60)
      findings.push(`Title is ${title.length} chars (optimal: 30-60).`);
    if (!metaDescription)
      findings.push("Missing meta description. Add 120-160 chars.");
    ...
    if (schemas.length === 0)
      findings.push("No JSON-LD schema. Add Organization/Article/FAQ schema.");
    if (externalLinkCount < 2)
      findings.push(
        `Only ${externalLinkCount} outbound links. Cite authoritative sources.`
      );
    if (!hasFAQSchema)
      findings.push("No FAQPage schema. FAQ data increases AI citation rates.");
    if (!hasHowToSchema)
      findings.push("No HowTo schema. Step-by-step data helps AI extraction.");
    if (!hasSnippetParagraph)
      findings.push(
        "No answer paragraphs (40-60 words). AI prefers self-contained blocks."
      );
    if (questionHeadingCount === 0)
      findings.push("No question headings (What/How/Why). Structure as Q&A.");
